...
if __name__ == "__main__":
    ...
    cst = Constraint(prj_path + f'/data/cst/{args.cst}.json')

    # Load the workloads
    with open(prj_path + f'/data/workload/{args.workload}.json') as f:
        data = json.load(f)
    workloads = []
    ...
    # arch_sol = search_record.arch_sol
    # # DSPs = (arch_sol['i_t1']/arch_sol['i_t2']) * (arch_sol['j_t1']/arch_sol['j_t2']) * (arch_sol['k_t2']) * 5
    # with open(results_csv, 'a') as f:
    #     # print(args.design_idx, search_record.cst['BRAM18K'], search_record.cst['DSP'], search_record.dsp_eff, search_record.throughput, file=f)
    #     print(  '{:<10}'.format(search_record.cst['BRAM18K']), \
    #             '{:<10}'.format(search_record.cst['DSP']), \
    #             # '{:<10}'.format(DSPs), \
    #             '{:.2%}'.format(search_record.dsp_eff), \
    #             '{:.2f}'.format(search_record.latency), \
    #             '{:.2f}'.format(search_record.throughput), file=f, sep=',')
    ...
    records = search_record.history
    for idx, record in enumerate(records):
        data_dict = record.to_dict()
        # save dict to json file wit indentation
        with open(f'{outdir}/history_{idx}.json', 'w') as f:
