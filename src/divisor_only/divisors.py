		...
		for i_t0 in i_t0_candidates:
			...
			for j_t0 in j_t0_candidates:
				...
				for k_t0 in k_t0_candidates:
					...
					top_results, is_k_updated, all_designs, valid_designs = mm_parallel_search(i_t0, j_t0, k_t0, candidates, design, top_results, resource_limits, num_top_results, objective, alpha, min_latency, min_off_chip)
					total_all_designs += all_designs
					total_valid_designs += valid_designs
					print(f'candidate ({i_t0}, {j_t0}, {k_t0})', 'current score = ', top_results[-1]['score'], \
			...
			for o_t0 in o_t0_candidates:
				...
				for r_t0 in r_t0_candidates:
					...
					for c_t0 in c_t0_candidates:
						...
						top_results, is_c_updated, all_designs, valid_designs = conv_parallel_search(i_t0, o_t0, r_t0, c_t0, P, Q, candidates, design, top_results, resource_limits, num_top_results, objective, alpha, min_latency, min_off_chip)
						total_all_designs += all_designs
						total_valid_designs += valid_designs
						print(f'candidate ({i_t0}, {o_t0}, {r_t0}, {c_t0})', 'current score = ', top_results[-1]['score'],\
...
if __name__ == '__main__':
	...
	# threshold
	threshold = 0#parser.parse_args().threshold
	...
	results_dir = prj_path + f'/results/{workload}/{problem_dims_str}/design_{permutation}_{dataflow}/{objective}/divisors'
	...
	import shutil
	new_file_name = f'{workload}_divisors_{problem_dims_str}_design_{permutation}_{dataflow}_{objective}_top_{num_top_results}_alpha_{alpha}_thr_{threshold}'
