...
def run_method(design, permutation, objective, workload, time_out, alpha):
	...
	os.system(prj_path + f'/src/randomized_methods/genetic_mp.sh {workload} {objective} {designs_dir} {time_out} {alpha}')
	return
...
if __name__ == '__main__':
	...
	results_dir = prj_path + f'/results/{workload}/{problem_dims_str}/design_{permutation}_{dataflow}/{objective}/genetic_mp'
	...
	for idx in design_iter:
		for objective in objectives:
			for trial in range(trials):
				...
				run_method(designs[idx], permutation, objective, workload, time_out, alpha)
	...
	import shutil
	new_file_name = f'{workload}_genetic_mp_{problem_dims_str}_design_{permutation}_{dataflow}_{objective}_alpha_{alpha}'
