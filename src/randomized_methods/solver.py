...
def off_chip_solver_gemm(search_task, cst, prj_path, fixed_params=None, save=0):
    """ If any parameter found in fixed_params, this parameter will not be tiled.
    """
    tmpdirname = f'{prj_path}/src/randomized_methods/solvers/mm'
    # if dir dosn't exist, create it
    ...
    with open(f'{tmpdirname}/tmp.run', 'w') as f:
        ...
        f.write('reset;\n')
        f.write(f'model {tmpdirname}/tmp.mod;\n')
        f.write(f'data {tmpdirname}/tmp.dat;\n')
        f.write('solve;\n')
    ...
    # if save == 1:
    #     shutil.copyfile(f'{tmpdirname}/tmp.mod', prj_path + '/solver/tmp.mod')
    #     shutil.copyfile(f'{tmpdirname}/tmp.dat', prj_path + '/solver/tmp.dat')
    #     shutil.copyfile(f'{tmpdirname}/tmp.run', prj_path + '/solver/tmp.run')
    
    return opt_dims

def off_chip_solver_conv(search_task, cst, prj_path, fixed_params=None, save=0):
    """ If any parameter found in fixed_params, this parameter will not be tiled.
    """
    tmpdirname = f'{prj_path}/src/randomized_methods/solvers/conv'
    if not os.path.exists(tmpdirname):
        os.makedirs(tmpdirname)
    ...
    if update != len(opt_dims):
        # The solver isn't finished correctly.
        raise RuntimeError("The solver isn't finished correctly.")
        opt_dims = None
...
def off_chip_solver(search_task, cst, fixed_params=None, save=0):
    ...
    if "gemm" in search_task.workload["tags"]:
        return off_chip_solver_gemm(search_task, cst, prj_path, fixed_params, save)
    elif "conv" in search_task.workload["tags"]:
        return off_chip_solver_conv(search_task, cst, prj_path, fixed_params, save)
    else:
