...
designs_dir = prj_path + '/gekko_solver/tmp/designs'
output_dir = prj_path + '/gekko_solver/tmp/outputs'
...
def print_model_params(f, desp, problem_size, resource_limits):
	...
	f.write('\n')
	f.write('def get_solver_latency(design, problem_size, resource_limits):\n')
	f.write('\tm = GEKKO()\n')
	...
	for p in desp["params"]:
		if p["name"] in problem_size:
			...
			f.write(f'\t{p["name"]} = m.Var(lb=1, ub={problem_dim})\n')
		elif 't2' in p["name"]:
			problem_dim = p["name"].split('_')[0]
			f.write(f'\t{p["name"]} = m.Var(lb=1, ub={problem_dim}_t1)\n')
		else:
			f.write(f'\t{p["name"]} = m.Var(lb=1, ub=16)\n')
	f.write('\n')
