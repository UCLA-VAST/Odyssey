...
def get_mm_candidates(i_t0, j_t0, k_t0, design):
	candidates = []
		...
		while(True):
			while(True):
				while(True):
					...
					candidates = get_mm_candidates(i_t0, j_t0, k_t0, design)
					time_end = time.time()
					candidate_time = time_end - time_start
					# search
					time_start = time.time()
					top_results, is_k_updated = parallel_search(candidates, design, top_results, resource_limits, num_top_results, objective, alpha, min_latency, min_off_chip)
					time_end = time.time()
...
if __name__ == '__main__':
	...
	file_name = f'{workload}/design_{design}_{problem_dims_str}_top_{num_top_results}_thr{threshold}_search_method_2'
