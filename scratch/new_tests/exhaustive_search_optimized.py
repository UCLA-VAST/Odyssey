...
def generate_mm_candidates(I, J, K, paddable_dims, design, resource_limits):
...
def search_mm(candidates, design, resource_limits, num_top_results):
	design_name = design.strip('.json')
	file_path = 'tmp/designs/register/' + design_name
	file_path = file_path.replace('/', '.')
	perf_model = importlib.import_module(file_path)
	...
	for idx in tqdm(range(len(candidates))):
		i_t0, j_t0, k_t0, i_t1_candidates, j_t1_candidates, k_t1_candidates = candidates[idx]
		for i_t1 in i_t1_candidates:
...
def search(design, paddable_dims, resource_limits, workload, problem_size, num_top_results, objective):
	...
	if workload == 'mm':
		...
		generator = generate_mm_candidates(I, J, K, paddable_dims, design, resource_limits)
		with multiprocessing.Pool(processes=num_processes) as p:
			results = p.imap_unordered(evaluate, [(params) for params in tqdm(generator)], chunksize=10)
		...
		# valid_results.sort(key=lambda x: x['cycles'][0])
		# valid_results = valid_results[:num_top_results]
		# valid_results = get_solution_info(valid_results, design, workload, problem_size)
	...
	# 			for k_t0 in k_t0_candidates:
	# 				k_t1_candidates = [x for x in range(1, k_t0+1) if k_t0 % x == 0]
	# 				# if cnt < 100 and k_t0 == 1:
	# 				# 	print(f'({i_t0}, {j_t0}, {k_t0})')
	# 				# 	print(f'\t{i_t1_candidates}')
	# 				# 	print(f'\t{j_t1_candidates}')
	# 				# 	print(f'\t{k_t1_candidates}')
	# 				# if k_t0 == 1:
	# 				# 	cnt += 1
	# 				candidates.append((i_t0, j_t0, k_t0, i_t1_candidates, j_t1_candidates, k_t1_candidates))
	# 	total_candidates = 0
	# 	for candidate in candidates:
	# 		i_t0, j_t0, k_t0, i_t1_candidates, j_t1_candidates, k_t1_candidates = candidate
	# 		total_candidates += len(i_t1_candidates) * len(j_t1_candidates) * len(k_t1_candidates)
	...
	# 	outputs = pool.starmap(search_mm, [(chunk, design, resource_limits, num_top_results) for chunk in chunks])
	# 	pool.close()
	# 	results = []
	# 	counts = []
	...
	# 	return top_results, sum(counts)
		...
		# for candidate in candidates:
		# 	all_candidate_count += len(candidate[3])*len(candidate[4])*len(candidate[5])
		# print('Total number of candidates: {}'.format(all_candidate_count))
		# exit()
		# for i_t0 in i_t0_candidates:
		...
		# 			candidates = get_mm_candidates(i_t0, j_t0, k_t0, i_t1_candidates, j_t1_candidates, k_t1_candidates, design)
		# 			print(f'Generated {len(candidates)} candidates for {i_t0}x{j_t0}x{k_t0}')
		# 			top_results, counts = parallel_search(candidates, design, top_results, resource_limits, num_top_results, objective, 0, 0, 0)
		# top_results = get_solution_info(top_results, design, workload, problem_size)
		# return top_results
		# search(candidates)
		# # shuffle candidates
	...
	# 	chunks = list_split(candidates, num_processes)
	# 	pool = multiprocessing.Pool(processes = num_processes)
	# 	results = pool.starmap(search_conv, [(i_t0, o_t0, r_t0, c_t0, p_t0, q_t0, idx, chunk, design, resource_limits, num_top_results) for idx, chunk in enumerate(chunks)])
	# 	pool.close()
	# 	results = [item for sublist in results for item in sublist]
	# 	results = sorted(results, key=lambda x: x['cycles'][0])
	# 	top_results = results[:num_top_results]
...
if __name__ == '__main__':
	...
	file_name = f'{workload}/design_{design}_{problem_dims_str}_top_{num_top_results}_exhaustive'
	results_file = prj_path + f'/scratch/new_tests/results/{file_name}.csv'
	log_file = prj_path + f'/scratch/new_tests/logs/{file_name}.log'
	...
	for idx in design_iter:
		for objective in objectives:
			for trial in range(1):
				...
				solutions, counts = search(designs[idx], paddable_dims, resource_limits, workload, problem_size, num_top_results, None)
				time_end = time.time()
