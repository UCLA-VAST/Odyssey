...
def get_mm_candidates(i_t0, j_t0, k_t0, k_new_factors, j_new_factors, i_new_factors, design):
	...
	i_t1_candidates = i_new_factors#[x for x in range(1, i_t0+1) if i_t0 % x == 0]
	j_t1_candidates = j_new_factors#[x for x in range(1, j_t0+1) if j_t0 % x == 0]
	k_t1_candidates = k_new_factors#[x for x in range(1, k_t0+1) if k_t0 % x == 0]
...
if __name__ == '__main__':
	...
	file_name = f'{workload}/design_{design}_{problem_dims_str}_top_{num_top_results}_thr{threshold}_exhaustive_pruned'
