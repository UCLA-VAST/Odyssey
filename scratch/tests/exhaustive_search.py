...
def search_mm_latency_off_chip(I, J, K, id, candidates, design, resource_limits, num_top_results, alpha, min_latency, min_off_chip):
	design_name = design.strip('.json')
	file_path = 'tmp/designs/register/' + design_name
	file_path = file_path.replace('/', '.')
	perf_model = importlib.import_module(file_path)
...
def search(design, paddable_dims, resource_limits, workload, problem_size, num_top_results, objective, alpha):
	...
	if workload == 'mm':
		...
		if objective == 'latency':
			...
			outputs = pool.starmap(search_mm_latency_off_chip, [(i_t0, j_t0, k_t0, idx, chunk, design, resource_limits, num_top_results, alpha, min_latency, min_off_chip) for idx, chunk in enumerate(chunks)])
			pool.close()
			results = []
			counts = []
			for output in outputs:
		...
		chunks = list_split(candidates, num_processes)
		pool = multiprocessing.Pool(processes = num_processes)
		outputs = pool.starmap(search_conv, [(i_t0, o_t0, r_t0, c_t0, p_t0, q_t0, idx, chunk, design, resource_limits, num_top_results) for idx, chunk in enumerate(chunks)])
		pool.close()
		results = []
		counts = []
		for output in outputs:
...
if __name__ == '__main__':
	...
	file_name = f'{workload}/design_{design}_{problem_dims_str}_top_{num_top_results}_alpha_{alpha}_obj_{objective}_exhaustive'
