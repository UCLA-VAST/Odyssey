...
def gen_solver_model(design_desc, problem_size, resource_limits):
	import copy
	# deep copy design_desc
	desc = copy.deepcopy(design_desc)
	design_name = design.strip('.json')
	file_path = 'tmp/designs/register/' + design_name + '.py'
	with open(file_path, 'a') as f:
		print_model_params(f, desc, problem_size, resource_limits)
		print_latency_est_func(f, desc)
		print_model_constraints(f, desc)
		print_objective(f, desc)
	desc = copy.deepcopy(design_desc)
...
for d_idx in range(first_design, last_design):
	...
	gen_solver_model(design_desc, problem_size, resource_limits)
	design_name = design.strip('.json')
	file_path = 'tmp/designs/register/' + design_name
	file_path = file_path.replace('/', '.')
	perf_model = importlib.import_module(file_path)
	...
	design_name = design.strip('.json')
	results_path = prj_path + f'/scratch_new/results/conv_{design_name}_{problem_dims_str}'
		...
		for o_t0 in range(O, O + 1 + paddings['o']):
			for r_t0 in range(R, R + 1 + paddings['r']):
				for c_t0 in range(C, C + 1 + paddings['c']):
					...
					solver_cycles = perf_model.try_all_solvers(padded_problem_size, resource_limits)
					if actual_cycles < solver_cycles:
						print('ERROR: actual cycles < solver cycles')
					if len(solver_cycles_list) > 0 and solver_cycles < solver_cycles_list[-1]:
						print('ERROR: solver cycles not monotonically increasing')
