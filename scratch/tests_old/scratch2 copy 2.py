...
for i in range(i_t0, i_t0 + 1 + i_padding):
	for j in range(j_t0, j_t0 + 1 + j_padding):
		...
		print()
		# results_nd = actual_bound_nd(i, j, k_t0, resource_limits)
		# for result in results_nd[:1]:
		# 	latency = result['cycles']
		# 	params = result['params']
		# 	i_t1 = params['i_t1']
		# 	j_t1 = params['j_t1']
		...
		# 	p11 = params['p11']
		# 	p12 = params['p12']
		# 	latency_meta = est_latency_no_ceil(params)[1]
		# 	resources = est_resource(params)[0]
		# 	DSPs = resources['DSP']
		# 	BRAM18Ks = resources['BRAM18K']
		# 	A_L2 = latency_meta["latency_main"]["A_IO_L2_in_latency"]
		# 	A_L3 = latency_meta["latency_main"]["A_IO_L3_in_latency"]
		# 	B_L2 = latency_meta["latency_main"]["B_IO_L2_in_latency"]
		# 	B_L3 = latency_meta["latency_main"]["B_IO_L3_in_latency"]
		# 	C_L1 = latency_meta["latency_main"]["C_drain_IO_L1_out_latency"]
		# 	C_L2 = latency_meta["latency_main"]["C_drain_IO_L2_out_latency"]
		# 	C_L3 = latency_meta["latency_main"]["C_drain_IO_L3_out_latency"]
		# 	PE = latency_meta["latency_main"]["PE_latency"]
		# 	bottleneck = max(latency_meta["latency_main"], key=latency_meta["latency_main"].get)
		# 	epilogue = latency_meta["epilogue"]
		# 	prologue = latency_meta["prologue"]
		# 	main = latency_meta["main"]
		# 	overall_bottleneck = latency_meta['bottleneck']
		# 	# print(bottleneck)
		# 	print(f'{i}, {j}, {k_t0}, {latency:.0f}, {i_t1}, {j_t1}, {k_t1}, {i_t2}, {j_t2}, {k_t2}, {p9}, {p10}, {p11}, {p12}, {DSPs}, {BRAM18Ks}, {PE:.1f}, {A_L2:.1f}, {A_L3:.1f}, {B_L2:.1f}, {B_L3:.1f}, {C_L1:.1f}, {C_L2:.1f}, {C_L3:.1f}, {bottleneck}, {epilogue:.1f}, {main:.1f}, {prologue:.1f}, {overall_bottleneck}')
		# print()
		# actual_nd_latencies.append(results_nd[0]['cycles'])

		# theoretical_latencies.append(theoretical_latency)
		actual_latencies.append(results[0]['cycles'])
		cycles_nd = get_lower_bound(i, j, k_t0, resource_limits)
		# params = {'i': i, 'j': j, 'k': k_t0, 'i_t1': i, 'j_t1': j, 'k_t1': k_t0, 'i_t2': 1, 'j_t2': 8, 'k_t2': 1, 'p9': 16, 'p10': 16, 'p11': 4, 'p12': 4}
		# cycles_nd = est_latency_no_ceil(params)[0]
		# resources_nd = est_resource(params)
		# print(f'({i}, {j}, {k_t0}) cycles: {cycles_nd:.0f}, params: {params}, DSPs: {resources_nd[0]["DSP"]}, BRAM18Ks: {resources_nd[0]["BRAM18K"]}')
		# print()
...
plt.plot(range(j_t0, j_t0 + 1 + j_padding), actual_nd_latencies, label='actual nd')
plt.legend()
plt.savefig('mm.png')
exit()
