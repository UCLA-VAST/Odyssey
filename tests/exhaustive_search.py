...
def search_conv_latency_off_chip(I, O, R, C, P, Q, id, candidates, design, resource_limits, num_top_results, alpha, min_latency, min_off_chip):
	design_name = design.strip('.json')
	file_path = 'tmp/designs/register/' + design_name
	file_path = file_path.replace('/', '.')
	perf_model = importlib.import_module(file_path)
...
def search(design, paddable_dims, resource_limits, workload, problem_size, num_top_results, objective, alpha):
	...
	if workload == 'mm':
		...
		if objective == 'latency':
			...
			outputs = pool.starmap(search_conv_latency_off_chip, [(i_t0, o_t0, r_t0, c_t0, p_t0, q_t0, idx, chunk, design, resource_limits, num_top_results, alpha, min_latency, min_off_chip) for idx, chunk in enumerate(chunks)])
			pool.close()
			results = []
			counts = []
			for output in outputs:
...
if __name__ == '__main__':
	...
	results_dir = prj_path + f'/results/{workload}/{problem_dims_str}/design_{design}/{objective}/exhaustive'
	...
	print('design_idx,objective,original workload,padded workload,fre,throughput (GFLOP/s),cycles,latency(ms),DSP eff,off-chip bytes,bandwidth (GB/s),CTC,DSPs,BRAMs,PEs,SA_dims,sa_sizes,params', file=csv_file)
	csv_file.close()
	id = 0
	...
	for idx in design_iter:
		for objective in objectives:
			for trial in range(1):
				...
				solutions, counts = search(designs[idx], paddable_dims, resource_limits, workload, problem_size, num_top_results, objective, alpha)
				time_end = time.time()
	...
	with open(log_file, 'w') as f:
		info = {'designs':counts, 'total_time':total_time_end-total_time_start}
