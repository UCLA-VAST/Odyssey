...
if __name__ == '__main__':
	...
	# load workload data
	with open(f'{prj_path}/tests/cnns/resnet50_new_tmp.json') as f:
		workloads_dict = json.load(f)
	# new_workloads = {'vgg16': []}
	# for wl in workloads_dict['workloads']:
	# 	new_workloads['vgg16'].append(str(wl['params']))
	# # save to json
	# with open(f'{prj_path}/tests/cnns/vgg16_new.json', 'w') as f:
	# 	json.dump(new_workloads, f, indent=1)
	...
	for idx in range(designs_per_dict):
		for name in workloads_dict:
			...
			cmd = f'python src/non_divisors.py        -w={workload} -p="{ps}" -obj={objective} -d="(1,5)" -a={alpha} -n={num_top_designs}   -thr={threshold}'
			os.system(cmd)
			print(f'Running non_divisors for {name} {ps}')
			cmd = f'python src/non_divisors.py        -w={workload} -p="{ps}" -obj={objective} -d="(1,6)" -a={alpha} -n={num_top_designs}   -thr={threshold}'
			os.system(cmd)
			print(f'Running non_divisors for {name} {ps}')
			cmd = f'python src/non_divisors.py        -w={workload} -p="{ps}" -obj={objective} -a={alpha} -n={num_top_designs}   -thr={threshold}'
			os.system(cmd)
			# odyssey
