...
def search(design, paddable_dims, resource_limits, workload, problem_size, num_top_results, objective, alpha, threshold):
	...
	if workload == 'mm':
		...
		j_counter = 0
		k_counter = 0
		i_threshold = ceil(threshold*I)
		j_threshold = ceil(threshold*J)
		k_threshold = ceil(threshold*K)
		...
		r_counter = 0
		c_counter = 0
		i_threshold = ceil(I*threshold)
		o_threshold = ceil(O*threshold)
		r_threshold = ceil(R*threshold)
		c_threshold = ceil(C*threshold)
...
if __name__ == '__main__':
	...
	results_dir = prj_path + f'/results/{workload}/{problem_dims_str}/design_{design}/{objective}/non_divisors'
