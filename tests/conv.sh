...
do
  ...
  do
    ...
    do
      ...
      python non_divisors.py        -w="conv" -p="{'i': $i, 'o' : $i, 'r' : $i, 'c' : $i, 'p' : 3, 'q' : 3}" -d=$d -obj='latency_off_chip' -a=$a -n=10       -thr=0.02
      # python odyssey.py             -w="conv" -p="{'i': $i, 'o' : $i, 'r' : $i, 'c' : $i, 'p' : 3, 'q' : 3}" -d=$d -obj='latency_off_chip' -a=$a --trials=10 -to=20 
      # python exhaustive_search.py   -w="conv" -p="{'i': $i, 'o' : $i, 'r' : $i, 'c' : $i, 'p' : 3, 'q' : 3}" -d=$d -obj='latency_off_chip' -a=$a -n=10 
    done
  done
done
