...
//  data_t A[I][K], B[K][J], C[I][J], C_golden[I][J]; 
...

  {
    ...
    std::vector<float, tapa::aligned_allocator<float>> dev_A_unserialized((1024) * (1024));
    std::vector<float, tapa::aligned_allocator<float>> dev_A(8388608);
    std::vector<float, tapa::aligned_allocator<float>> dev_B_unserialized((1024) * (1024));
    std::vector<float, tapa::aligned_allocator<float>> dev_B(16777216);
    std::vector<float, tapa::aligned_allocator<float>> dev_C_unserialized((1024) * (1024));
    std::vector<float, tapa::aligned_allocator<float>> dev_C(1048576);

    // Initialize host buffers
    std::copy(reinterpret_cast<float *>(A), reinterpret_cast<float *>(A) + (1024) * (1024), dev_A_unserialized.begin());
