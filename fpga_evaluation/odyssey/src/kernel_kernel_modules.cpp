...
#pragma HLS INLINE
  ...
  for (int c4 = p1; c4 <= 42; c4 += 1) {
    // io_L1
    if (c4 == p1) {
      for (int c5 = 0; c5 <= 2; c5 += 1) {
        // access_coalesce
        for (int c6 = 0; c6 <= 12; c6 += 1) {
          // hls_pipeline
          {
            C_t2 in_data;
            C_t2 out_data;
            in_data = local_C[c5][2 * c6 / 2];
            out_data = in_data;
